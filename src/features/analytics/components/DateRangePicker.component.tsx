...
export default function DateRangePicker({ onDateRangeChange }: DateRangePickerProps) {
  ...
  const handleCustomDateChange = () => {
    const hasValidDates = startDate.getTime() !== EMPTY_DATE.getTime() && endDate.getTime() !== EMPTY_DATE.getTime();
    if (hasValidDates) {
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <DatePicker
            label="Start Date"
            value={startDate}
            onChange={(newValue: Date | null) => setStartDate(newValue ?? EMPTY_DATE)}
            maxDate={endDate}
          ...
          <DatePicker
            label="End Date"
            value={endDate}
            onChange={(newValue: Date | null) => setEndDate(newValue ?? EMPTY_DATE)}
            minDate={startDate}
            maxDate={new Date()}
            slotProps={{ textField: { fullWidth: true } }}
          />
